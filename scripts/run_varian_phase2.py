#!/usr/bin/env python3
"""Phase 2 Clinical Varian pipeline: downsample → DRR → compress → DVF → prep_train."""
...
from varian_drr import patient_num_from_scan_id, varian_drr_opts_for_scan  # noqa: E402

DEFAULT_SCAN = os.environ.get("SPARE_SCAN_ID", "CV_P1_V_01")
...
def run_drr(train: Path, geom_xml: Path, scan_id: str) -> int:
    ...
    for i, mha in enumerate(mhas, start=1):
        ...
        logging.info("DRR %s (%d/%d) detector=%s", mha.name, i, len(mhas), det)
        ok, err = run_drr_fn(
            str(mha),
            str(train),
...
def main() -> int:
    p = argparse.ArgumentParser(description="Clinical Varian SPARE phase 2 pipeline")
    ...
    logging.basicConfig(
        ...
        handlers=[
            ...
            logging.FileHandler(run_root / "logs" / "phase2.log", mode="a"),
        ],
    )
    ...
    if not args.skip_drr:
        opts = varian_drr_opts_for_scan(train, scan_id)
        logging.info("=== DRR (Varian %s, %s) ===", opts["detector_size_xy"], geom_xml.name)
        run_drr(train, geom_xml, scan_id)
